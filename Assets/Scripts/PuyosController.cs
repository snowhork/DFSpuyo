    ...
    public class PuyosController
    {
        ...
        public void Execute()
        {
            ...
            foreach (var puyo in _puyos) //_puyosをforeachしたときは、puyoはnullにならない仕様です
            {
                ...
                WFS(puyo.X, puyo.Y, flags, queue, list); //訪れていないので探索を始めます。

                if(list.Count < 4) continue;
                foreach (var puyo2 in list)
                {
                    puyo2.Vanish();
                }
            }
        ...
        private void WFS(int x, int y, bool[,] flags, Queue<Puyo> queue, List<Puyo> list)
        {
            // この実装では、(x,y)のぷよはnullでないことを前提としています。
            flags[x, y] = true; // 訪れたフラグを立てる
            list.Add(_puyos.Get(x, y));
            ...
            if(puyo != null && !flags[puyo.X, puyo.Y] && color == puyo.Color) queue.Enqueue(puyo);
            ...
            if(next != null) WFS(next.X, next.Y, flags, queue, list);
        }
    }
}
